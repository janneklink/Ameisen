...
def __main__():
    ...
    spielfeld = Karte.Karte(nestposition_x,nestposition_y,futterquellen_anzahl)#Das spielfeld auf dem sich die Ameisen bewegen
    ameisenzahl=100# die variable Ameisenanzahl
    ameisen_erstellen(spielfeld.nest, ameisenliste,ameisenzahl)#Hier wird die Funktion aufgerufen, welche di Ameisen erstellt
...
def ameisen_erstellen(startpunkt, ameisenliste,ameisenanzahl):
    for ameisen_nr in range(0, ameisenanzahl):#Nun wird für jeden integer von 0 bis zur endgültigen ameisen anzahl eine Ameise erstellt
        ameise = Ameisen.ameise_erstellen(ameisen_nr, startpunkt)#das Ameisenobjekt wird erstellt
